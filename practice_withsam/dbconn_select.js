...
var db = mysql.createConnection({  // mysql db에 접속하기 위한 접속정보를 저장한다.
...
db.connect();  // 데이터베이스 접속정보를 가지고 mysql server에 접속한다.
...
db.query( sql, 
                function (e, r, fields) {
                    if (e) throw e;// sql 실행하고 에러가 있으면 에러를 출력하고
                    else {              // 에러가 없으면 results 배열에 담겨있는 객체정보를 확인해서 콘솔에 출력한다.
                        console.log(r);
                        console.log('The solution is: ', r[1].bookname);
                    }
                }
                );

const sql2 = 'SELECT * from customer;';   // customer table에서 고객이름을 가져오기 위한 sql 문을 변수에 저장함
db.query(    // sql2의 실행결과를 콜백함수의 매개변수 즉 error, results 로 넘겨준다.
                sql2, 
...
db.query( sql3,                   // sql3의 실행결과를 콜백함수의 매개변수 즉 error, results 로 넘겨준다.
            function (error, results) { 
                if (error) throw error;   // sql 실행하고 에러가 있으면 에러를 출력하고
                    ...
                    let res = results[0];
                    console.log('The solution is: ', res.orderdate);
                }   
            });
